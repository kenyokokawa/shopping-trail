...
(function () {
  ...
  function isAmazon(url) {
    return /amazon\.(com|co\.uk|ca|de|co\.jp|fr|es|it|com\.au|in|com\.br|com\.mx)/.test(url);
  }
  ...
  function extractAmazon() {
    ...
    let description = '';
    if (bullets.length > 0) {
      description = Array.from(bullets).map(b => b.textContent?.trim()).filter(Boolean).slice(0, 5).join(' | ');
    }

    const image = getImage('#landingImage') || getImage('#imgBlkFront') || getImage('.a-dynamic-image');
    const price = getText('.a-price .a-offscreen') || getText('#priceblock_ourprice') || getText('#corePrice_feature_div .a-offscreen');
  ...
  function extractEbay() {
    ...
    let title = getText('h1.x-item-title__mainTitle span') || getText('h1[itemprop="name"]');
    if (!title) return null;
    title = title.replace(/^Details about\s+/i, '');

    const image = getImage('.ux-image-carousel-item.active img') || getImage('.ux-image-carousel-item img');
    const price = getText('.x-price-primary span[itemprop="price"]') || getText('.x-price-primary .ux-textspans');
  ...
  function extractWalmart() {
    ...
    const title = getText('h1[itemprop="name"]') || getText('[data-testid="product-title"]');
    if (!title) return null;

    const highlights = document.querySelectorAll('[data-testid="product-highlights"] li');
    let description = '';
    if (highlights.length > 0) {
      description = Array.from(highlights).map(h => h.textContent?.trim()).filter(Boolean).slice(0, 5).join(' | ');
    }

    const image = getImage('[data-testid="hero-image-container"] img') || getImage('.hover-zoom-hero-image img');
  ...
  function extractTarget() {
    if (!/\/p\//.test(window.location.pathname) && !/\/A-\d+/.test(window.location.pathname)) {
      return null;
    }

    const title = getText('h1[data-test="product-title"]') || getText('[data-test="product-title"]');
    if (!title) return null;

    const highlights = document.querySelectorAll('[data-test="product-highlights"] li');
    let description = '';
    if (highlights.length > 0) {
      description = Array.from(highlights).map(h => h.textContent?.trim()).filter(Boolean).slice(0, 5).join(' | ');
    }

    const image = getImage('[data-test="product-image"] img') || getImage('.slideDeckPicture img');
  ...
  function extractBestBuy() {
    if (!/\/site\/.*\/\d+\.p/.test(window.location.pathname) && !document.querySelector('[data-sku-id]')) {
      return null;
    }
    ...
    if (features.length > 0) {
      description = Array.from(features).map(f => f.textContent?.trim()).filter(Boolean).slice(0, 5).join(' | ');
    }

    const image = getImage('.primary-image img') || getImage('[data-track="primary-image"] img');
    const price = getText('[data-testid="customer-price"] span') || getText('.priceView-customer-price span');
  ...
  function extractMicrodata() {
    ...
    return {
      title: name,
      description: (descEl?.textContent?.trim() || descEl?.getAttribute('content') || '').substring(0, 300),
      image: imageEl?.src || imageEl?.getAttribute('content') || '',
      price: priceEl?.textContent?.trim() || priceEl?.getAttribute('content') || '',
      currency: currencyEl?.textContent?.trim() || currencyEl?.getAttribute('content') || 'USD',
      url: window.location.href,
      site: extractSiteName()
    };
  }
